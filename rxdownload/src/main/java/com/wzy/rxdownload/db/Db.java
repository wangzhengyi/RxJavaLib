...
public class Db {
    ...
    static final class RecordTable {
        ...
        static DownloadStatus readStatus(Cursor cursor) {
            boolean isChunked = cursor.getInt(cursor.getColumnIndex(COLUMN_IS_CHUNKED)) > 0;
            long downloadSize = cursor.getLong(cursor.getColumnIndex(COLUMN_DOWNLOAD_SIZE));
            long totalSize = cursor.getLong(cursor.getColumnIndex(COLUMN_TOTAL_SIZE));
            return new DownloadStatus(isChunked, downloadSize, totalSize);
        }

        static DownloadRecord read(Cursor cursor) {
            DownloadRecord record = new DownloadRecord();
            record.setUrl(cursor.getString(cursor.getColumnIndex(COLUMN_URL)));
            record.setSaveName(cursor.getString(cursor.getColumnIndex(COLUMN_SAVE_NAME)));
            record.setSavePath(cursor.getString(cursor.getColumnIndex(COLUMN_SAVE_PATH)));
            ...
            record.setFlag(cursor.getInt(cursor.getColumnIndex(COLUMN_DOWNLOAD_FLAG)));
